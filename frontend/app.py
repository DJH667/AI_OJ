...
def render_languages_page() -> None:
    ...
    st.markdown(
        ...
        "⚠ **注册只登记命令模板，不会自动安装解释器/编译器**：提交评测时后端会按你填写的命令调用系统工具，"
        "若运行环境没有安装对应工具（如系统未装 go 编译器），提交会评测失败（error/UNK）。"
        "内置的 python/cpp 已随评测环境就绪；注册 go/java/c 等语言前，请先确认评测环境（WSL Ubuntu）已安装对应工具链。\n\n"
