...
- 题目管理（所有登录用户可见）：右上角「AI 命题」「语言管理」与「＋ 新增题目」按钮；支持按编号/标题关键词搜索；每题卡片右侧编辑/删除图标；
  - **语言管理**：列出已注册语言，任意登录用户可注册新语言（name/file_ext/run_cmd 必填，compile_cmd 与 time/memory_limit 可选；{src}/{exe} 会被替换为路径）；⚠ 注册仅登记命令模板、**不安装解释器/编译器**——提交评测时按命令调用系统工具，环境未装对应工具会评测失败（error/UNK），注册前请先在评测环境（WSL Ubuntu）安装工具链（内置 python/cpp 已就绪）；
...
1. **模型配置**（折叠面板，per-user）：**模型为下拉选择题**（目录实时取自 OpenRouter，并内置补充 **DeepSeek 官方直连模型** `deepseek-v4-flash`/`deepseek-v4-pro`/`deepseek-v4-flash-vision-exp` 与**千问官方直连模型** `qwen-turbo`/`qwen-plus`/`qwen-max`；离线用内置常用模型兜底；也可选"自定义模型…"手填 id；旧 id `deepseek-chat`/`deepseek-reasoner` 已弃用）；选中后自动展示该模型**真实单价**（USD/1M tokens）与**自动汇率**（Frankfurter/ECB 实时获取，离线用内置参考值兜底，失败 5 分钟后自动重试）——单价/汇率均无需手工填写；只需填 `api_key`（`provider_url` 在"高级"折叠里可改，默认 OpenRouter）。**不填 key 时自动走本地 mock**（也可完整演示）。
   - **OpenRouter**：`provider_url` 填 `https://openrouter.ai/api/v1`，模型用厂商前缀 id（如 `deepseek/deepseek-v4-pro`），Key 用 OpenRouter 的 key；
   - **DeepSeek 官方**：`provider_url` 填 `https://api.deepseek.com`，模型选 `deepseek-v4-flash` / `deepseek-v4-pro`（不带前缀），Key 用 DeepSeek 官网 key；
   - **千问官方**：`provider_url` 填 `https://dashscope.aliyuncs.com/compatible-mode/v1`，模型选 `qwen-turbo` / `qwen-plus` / `qwen-max`（不带前缀），Key 用阿里云百炼官网 key；混用（OpenRouter 模型 id 配官方地址）会报 Model Not Exist。
2. **命题输入**（二选一）：
   - 结构化表单：**语言必选**（=已注册语言下拉）、考点多选（可自定义）、难度分、预期复杂度（下拉含**"不考察复杂度"**，选中后不卡高复杂度算法、强化边界/极端用例）、数据规模、情景/备注；可选"站内参考题"；
