...
- **对拍引擎**（仅硬核，兼容所有已注册语言）：生成器产**多档规模**数据（小/中/大，按期望复杂度推导，使 O(N²) 类中小点可过、大点 TLE → 部分分梯度）→ 标答算 expected → 小点暴力对照 → 通过采纳；不一致 → 错误摘要回传 AI 重试 ≤retry_limit → 用尽：completed + **needs_review** + 错误摘要（题目不入库）。复用 judge 编译/运行原语（services/runner.py）。
...
16. **AI 硬核模式**（同）：对拍可选（默认关）；开启后三代码对拍闭环 + 重试次数可调（默认 2，用尽标记 needs_review 不入库）。
