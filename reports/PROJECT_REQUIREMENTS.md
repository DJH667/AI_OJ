...
8. **测试支持接口**：`POST /api/reset/`（仅管理员，测试环境可不校验）——清空用户/题目/提交数据、退出登录、重建初始管理员。**自动评测会使用，必须实现**。⚠ 实现语义：**默认按仅管理员鉴权**——未登录 401、已登录非管理员 403（api.md 异常表含 401/403，异常顺序 401>403）；"测试环境可不校验"由 `config.RESET_REQUIRE_ADMIN` 开关支持（默认 True，若评测确需免登录调用可置 False）。**已确认（2026-09-04）：自动评测会先登录 admin 再调用 reset，默认 True 正确、无需放宽。** reset 同时把动态注册语言重置回内置 python/cpp（回到初始环境），但**用户动态注册新语言的接口能力（`POST /api/languages/`）不受影响、始终保留**（用户指示 2026-09-03）。
...
- `PUT /api/users/{user_id}/role`：仅管理员，role∈{admin,user,banned}（否则 400），**记录权限操作日志**（谁在何时改了谁的权限——与 Step5 的 access 日志审计（action=`view_logs`）是两回事，勿混淆）。
...
- **纯文本**：用自然语言描述命题需求；可选**本地上传文件**作为背景资料——若所接入模型为纯文本模型，上传界面须警告"大模型无法观看图片"（文件仅按其可提取的文本内容处理）。

**站内参考题（现状 2026-09-09，按现有实现如实说明）**：结构化表单提供"站内参考题"下拉，可**单选一道**站内题作为模型上下文（该题题面/配置随需求送入模型）；**纯文本不解析站内题目链接、暂不实现按 `difficulty_score` 邻域自动抽取相近题**（时间紧，用户拍板保持现状）。参考题内容仅作为喂给模型的上下文，不直接成为产出。
...
### 6.2 AI 模块追加定案（2026-09-05 问答定稿，见 reports/ai-alignment-notes.md）

- **语言必选**：命题结构化输入含必选"语言"，选项 = **全部已注册语言**（动态取 `GET /api/languages/`，默认 python）。
- **纯文本语言双层防护**：prompt 要求模型遇未注册语言直接产出"语言不支持"错误；后端解析兜底校验（language ∉ 已注册 → 任务失败并返回原因+可用列表）。
- **硬核模式（可选对拍，默认关）**：前端开关，开启后显示对拍重试次数调节（默认 2）；开启执行**三代码对拍闭环**（数据生成器/标答/暴力，均用所选语言），关闭则普通生成（AI 直接给全量测例）。
- **对拍引擎**（仅硬核，兼容所有已注册语言）：生成器产**多档规模**数据（小/中/大，按期望复杂度推导，使 O(N²) 类中小点可过、大点 TLE → 部分分梯度）→ 标答算 expected → 小点暴力对照 → 通过采纳；不一致 → 错误摘要回传 AI 重试 ≤retry_limit → 用尽：completed + **review** + 错误摘要（题目不入库）。复用 judge 编译/运行原语（services/runner.py）。
- **落库内容**：题目采纳 = 题目字段 + `samples`（展示）+ **`testcases` 全量（对拍通过/普通生成，含大数据文本）** + 私有 `difficulty_score` 与 `ai_meta`（语言/生成器/标答/暴力代码/重试次数，不进任何对外 API）。
- **进度**：任务状态机 + 轮询展示（R3）；cancel 真实终止。
- 测试数据底线：不得有错误数据（对拍/人工保障）。
...
5. **AI 命题界面**：结构化表单（考点多选/难度数值/预期复杂度/数据规模/背景/备注，**不含 SPJ 维度**）与纯文本两种输入；结构化表单可选一道站内参考题（2026-09-09 起按现状：不做链接解析与邻域抽题）；上传文件须对纯文本模型提示"无法观看图片"；AI 产出预填"新增/编辑题目"表单供人审改（详见 §6.1）。
...
15. **AI 语言必选**（问答定案 2026-09-05）：命题语言为必选项，选项=全部已注册语言（默认 python）。
16. **AI 硬核模式**（同）：对拍可选（默认关）；开启后三代码对拍闭环 + 重试次数可调（默认 2，用尽标记 review 不入库）。
