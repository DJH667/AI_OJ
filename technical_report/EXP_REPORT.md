...
5. **多通道并行**：会话共4部分：推进部分、评审部分、打磨部分、讲解部分。其中推进部分负责搭建可运行的demo作为骨架，打磨部分在此基础上完成最终项目，评审部分负责评估并记录代码中可能的风险和与文档不符、实现不全之处。讲解部分负责与我同步项目信息。


### 5.3 Vibe Coding 代码比例

本项目所有代码均由AI完成，人工负责出现问题时审阅并反馈相关代码以及报告的撰写
...
4. **并发写与集中状态**：全 JSON 存储在单用户串行下够用，但多用户并发/多进程时需引入锁或数据库；内存/超时采样间隔与并发开销需平衡。但单机没有对应测试环境。
