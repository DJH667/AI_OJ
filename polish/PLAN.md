...
## 7. polish 第二轮（2026-09-08 用户反馈）

已实施：

- 题目编号对用户可见：题库卡片 `#{编号}` 徽章、详情页 `题目编号` caption；AI 命题"站内参考题"下拉格式 `{编号} {标题}`（编号 ljust(12) 对齐）。
- 通过率条改为无数字文本的纯条状。
- 查询页：状态筛选（全部/评测中/通过/部分通过/未通过/错误）；`GET /api/submissions/?scope=all`（仅管理员）一次性取全量，消除逐用户 N+1 与 1000 条截断；查询结果每 3s 自动刷新。
- 提交详情：代码按提交语言高亮（Pygments lexer 映射）；管理员可"重新评测"（`PUT /api/submissions/{id}/rejudge`）。
- 前端会话级 TTL 缓存（30s）题目/语言列表，增删改/提交/审批/重评后统一清缓存。
- `polish/app_smoke.py` 固化至 `scripts/app_smoke.py`；README 更新前端页面说明与冒烟命令。

验证：后端 pytest 71 passed（2 个评测执行用例在 Windows 环境不可用：cpp 编译 AC、MLE 内存限制，需 WSL/Linux 跑全量）；前端 AppTest 冒烟 PASSED。
